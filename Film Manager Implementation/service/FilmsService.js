    ...
    return new Promise((resolve, reject) => {
      ...
      db.run(sql, [film.title, owner, film.private, film.watchDate, film.rating, film.favorite], function(err) {
          if (err) {
              ...
              //send a retained message to the MQTT broker for the new film, if public
              if(!film.private){
                      ...
                      if (err)
                          ...
                          db.run(sql4, [filmId], (err) => {
                              if (err)
                                  ...
                                  // delete the retained message for the film from the MQTT broker
                                  mqtt.publishFilmMessage(filmId, new MQTTFilmMessage("deleted", null, null));
                                  //mqtt.publishFilmMessage(filmId, null); //uncomment if we want to clear the last retained message
